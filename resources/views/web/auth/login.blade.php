<x-module-release-1::auth-layout :title="__('Login | Module Release 1')">
    ...
    <div class="rounded-xl pb-8 pt-6 px-8 shadow-2xl bg-white dark:bg-slate-900 animate-scale-in max-w-xl">
        <div>
            ...
            <form action="{{ route(module_release_1_meta('kebab').'.login') }}" method="POST" class="space-y-4">
                @csrf
                @php
                ...
                <div class="flex items-start justify-between">
                    <x-link href="{{ route(module_release_1_meta('kebab').'.password.request') }}" intent="ghost">
                        @lang('Forgot Password?')
                    </x-link>
                    <x-link href="{{ route(module_release_1_meta('kebab').'.register') }}" intent="ghost">
                        @lang('Register')
                    </x-link>
                </div>
            </form>
