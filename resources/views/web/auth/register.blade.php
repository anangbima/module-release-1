<x-module-release-1::auth-layout :title="__('Regiter | Module Release 1')">
    ...
    <div class="rounded-xl pb-8 pt-6 px-8 shadow-2xl bg-white dark:bg-slate-900 animate-scale-in max-w-4xl">
        <div>
            ...
            <form action="{{ route(module_release_1_meta('kebab').'.register') }}" method="POST" class="space-y-4">
                @csrf
                @php
                ...
                <div class="flex items-start justify-end">
                    <x-link href="{{ route(module_release_1_meta('kebab').'.login') }}" intent="ghost">
                        @lang('Login')
                    </x-link>
                </div>
            </form>
