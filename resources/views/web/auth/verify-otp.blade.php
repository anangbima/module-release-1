<x-module-release-1::auth-layout :title="__('Verify OTP | Module Release 1')">
    ...
    <div class="rounded-xl pb-8 pt-6 px-6 sm:px-8 shadow-2xl bg-white dark:bg-slate-900 animate-scale-in max-w-xl w-full mx-auto">
        <div>
            ...
            {{-- OTP Form --}}
            <form id="otp-form" action="{{ route(module_release_1_meta('kebab').'.verify-otp') }}" method="POST" class="space-y-4 mt-6">
                @csrf
            ...
            {{-- Resend --}}
            <form action="{{ route(module_release_1_meta('kebab').'.verify-otp.resend') }}" method="POST" class="mt-4 text-end">
                @csrf
