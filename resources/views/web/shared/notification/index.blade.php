<x-dynamic-component
    :component="'module-release-1::' . $role . '-layout'"
    :title="__($title)"
    :role="$role"
...
>
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex gap-2 items-center mb-4 mt-1">
            <x-link 
                href="{{ route(module_release_1_meta('kebab').'.'.$role.'.notifications.index', ['filter' => 'all']) }}"
                intent="primary" 
            ...
            <x-link 
                href="{{ route(module_release_1_meta('kebab').'.'.$role.'.notifications.index', ['filter' => 'read']) }}"
                intent="primary"
            ...
            <x-link 
                href="{{ route(module_release_1_meta('kebab').'.'.$role.'.notifications.index', ['filter' => 'unread']) }}"
                intent="primary"
        ...
        <div class="flex gap-2 items-center">
            {{-- Mark all as read --}}
            <form method="POST" action="{{ route('module-release-1.'.$role.'.notifications.mark-all-as-read') }}">
                @csrf
                <x-link 
    ...
    @forelse ($notifications as $section => $items)
        <div class="mb-8 pb-4 border-b border-gray-200 dark:border-gray-700">
            ...
            @forelse ($items as $notification)
                <a 
                    href="{{ route('module-release-1.'.$role.'.notifications.show', $notification->id) }}" 
                    ...
                    <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                        <div>
                            @if (!$notification->read_at) 
                                <form method="POST" action="{{ route('module-release-1.'.$role.'.notifications.mark-as-read', $notification->id) }}">
                                    @csrf
                        ...
                        <div>
                            <form method="POST" action="{{ route('module-release-1.'.$role.'.notifications.destroy', $notification->id) }}">
                                @csrf
                                @method('DELETE')
    ...
    >
        <div>
            ...
            <div class="w-full flex justify-end gap-3 mt-4">
                ...
                <form method="POST" action="{{ route('module-release-1.'.$role.'.notifications.clear-all') }}">
                    @csrf
