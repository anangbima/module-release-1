...
namespace Modules\ModuleRelease1\Services\Auth;
...
use Modules\ModuleRelease1\Notifications\OtpNotification;
...
class AuthenticationService
{
    ...
    public function login(Request $request)
    {
        ...
        $user = Auth::guard(module_release_1_meta('snake').'_web')->user();
    ...
    public function sendOtp($user)
    {
        ...
        Auth::guard(module_release_1_meta('snake').'_web')->logout();
    }
    ...
    public function confirmOtp($userId, $inputOtp)
    {
        ...
        Auth::guard(module_release_1_meta('snake').'_web')->login($user);
    ...
    {
        $guard = module_release_1_meta('snake').'_web'; // atau release
    ...
    public function register(Request $request)
    {
        ...
        Auth::guard(module_release_1_meta('snake').'_web')->login($userCreated);
    ...
    public function confirmPassword(string $password)
    {
        ...
        if (!Auth::guard(module_release_1_meta('snake').'_web')->validate([
            'email' => $user->email,
            'password' => $password,
        ])) {
            throw ValidationException::withMessages([
    ...
    {
        return Password::broker(module_release_1_meta('snake').'_users')->reset(
            $credentials,
    ...
    {
        $status = Password::broker(module_release_1_meta('snake').'_users')->sendResetLink([
            'email' => $email,
        ]);
